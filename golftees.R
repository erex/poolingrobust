...
pdf(file="./figures/teeplot.pdf",h=6, w=10)
attach(gex.data)
...
detach(glines)
attach(gbnd)
for(i in 1:(length(gbnd[,1])-1)) lines(c(x[i],x[i+1]),c(y[i],y[i+1]),col=type[i]*Stratum[i]+1,lwd=2,lty=2)
dev.off()

seen = apply(gex.data[,16:23],1,sum) # number times seen by anyone
pdf(file="./figures/seenplot.pdf",h=6, w=10)
plot(abs(gex.data$SignPerp),seen>0,xlab="Perpendicular distance",ylab="Seen") # seen at all vs perp dist
dev.off()
